...
/**
 * ImageGalleryView Modal Component
      ...
      replies: [
        {
          ...
          authorImage: 'https://placehold.co/40x40',
          timeAgo: '5h',
          content: 'you can use YouTube. It\'s not a big deal if your JavaScript foundation is solid',
          isAuthor: true
        }
      ]
    }
      ...
      <DialogContent sx={{ padding: 0, position: 'relative', overflow: 'hidden', height: '100%' }}>
        ...
        <div className={`flex ${isMobile ? 'flex-col' : ''} h-full`}>
          ...
          <div className={`${isMobile ? 'w-full' : 'w-1/2'} flex flex-col bg-black`}>
            {/* Main Image Display */}
            <div className={`${isMobile ? 'h-[40vh]' : 'flex-1'} relative flex items-center justify-center ${isMobile ? 'p-2' : 'p-4'}`}>
              <img
              ...
              {/* Image Counter */}
              <div className={`absolute ${isMobile ? 'bottom-2' : 'bottom-4'} left-1/2 transform -translate-x-1/2 bg-black bg-opacity-60 text-white ${isMobile ? 'px-3 py-1 text-xs' : 'px-4 py-2 text-sm'} rounded-full`}>
            ...
            <div className={`bg-gray-900 ${isMobile ? 'p-2' : 'p-4'} overflow-x-auto`}>
              <div className={`flex ${isMobile ? 'gap-1' : 'gap-2'} justify-start ${isMobile ? '' : 'justify-center'}`}>
          ...
          <div className={`${isMobile ? 'w-full' : 'w-1/2'} bg-white flex flex-col ${isMobile ? 'h-[60vh]' : ''}`}>
            {/* Property Header */}
            ...
            <div className={`${isMobile ? 'p-3' : 'p-6'} border-b bg-gray-50`}>
              <Typography variant={isMobile ? 'caption' : 'subtitle2'} className="font-semibold mb-2">
                Add a comment
              </Typography>
              <div className={`${isMobile ? 'mb-2' : 'mb-3'}`}>
                <Typography variant={isMobile ? 'caption' : 'body2'} className="text-gray-700 mb-1">
                  Your Rating
                </Typography>
                <Rating
                  value={rating}
              ...
              <div className={`flex ${isMobile ? 'gap-1' : 'gap-2'}`}>
                <Avatar sx={{ width: isMobile ? 28 : 32, height: isMobile ? 28 : 32, fontSize: isMobile ? '0.875rem' : '1rem' }}>U</Avatar>
            ...
            <div className={`flex-1 overflow-y-auto ${isMobile ? 'p-3' : 'p-6'}`}>
              <div className={`flex items-center justify-between ${isMobile ? 'mb-2' : 'mb-4'}`}>
                <Typography variant={isMobile ? 'caption' : 'subtitle2'} className="font-semibold">
                  Comments ({reviews.length})
                </Typography>
                <select className={`${isMobile ? 'text-xs' : 'text-sm'} text-gray-600 border-none outline-none cursor-pointer`}>
                ...
                {reviews.map((review) => (
                  <div key={review.id} className={`${isMobile ? 'space-y-1' : 'space-y-2'}`}>
                    <div className={`flex ${isMobile ? 'gap-2' : 'gap-3'}`}>
                      ...
                      <div className="flex-1">
                        <div className="flex items-center gap-2 mb-1">
                          <Typography variant={isMobile ? 'caption' : 'body2'} className="font-semibold">
                            {review.author}
                          </Typography>
                          <Typography variant="caption" className="text-gray-500" sx={{ fontSize: isMobile ? '0.65rem' : '0.75rem' }}>
                            • {review.timeAgo}
                          </Typography>
                          {review.isAuthor && (
                            <span className={`${isMobile ? 'text-[10px]' : 'text-xs'} bg-orange-100 text-orange-600 px-2 py-0.5 rounded`}>
                              Author
                            </span>
                          )}
                        </div>
                        ...
                        <Typography variant={isMobile ? 'caption' : 'body2'} className="text-gray-700">
                          {review.content}
                        </Typography>
                        <div className={`flex items-center ${isMobile ? 'gap-2 mt-1' : 'gap-4 mt-2'}`}>
                          <button className={`flex items-center gap-1 text-gray-600 hover:text-orange-600 ${isMobile ? 'text-xs' : 'text-sm'}`}>
                    ...
                    {review.replies && review.replies.length > 0 && (
                      <div className={`${isMobile ? 'ml-8 space-y-2 mt-2' : 'ml-12 space-y-3 mt-3'}`}>
                          ...
                          <div key={reply.id} className={`flex ${isMobile ? 'gap-2' : 'gap-3'}`}>
                            ...
                            <div className="flex-1">
                              <div className={`flex items-center ${isMobile ? 'gap-1' : 'gap-2'} mb-1`}>
                                <Typography variant={isMobile ? 'caption' : 'body2'} className="font-semibold" sx={{ fontSize: isMobile ? '0.75rem' : '0.875rem' }}>
                                  {reply.author}
                                </Typography>
                                <Typography variant="caption" className="text-gray-500" sx={{ fontSize: isMobile ? '0.65rem' : '0.75rem' }}>
                                  • {reply.timeAgo}
                              ...
                              <Typography variant={isMobile ? 'caption' : 'body2'} className="text-gray-700" sx={{ fontSize: isMobile ? '0.75rem' : '0.875rem' }}>
                                {reply.content}
                              </Typography>
                              <div className={`flex items-center ${isMobile ? 'gap-2' : 'gap-4'} mt-1`}>
                                <button className={`text-gray-600 hover:text-orange-600 ${isMobile ? 'text-[10px]' : 'text-xs'}`}>
                                  Like
                                </button>
                                <button className={`text-gray-600 hover:text-orange-600 ${isMobile ? 'text-[10px]' : 'text-xs'}`}>
                                  Reply
