...
function TsFileRenamingMigrationDoc() {
  return (
    <>
      <Typography variant="h4" className="mb-16 font-700">
      ...
      <Typography className="mb-16" variant="body1" gutterBottom>
        When migrating a project from JavaScript (JS) to TypeScript (TS), one of the initial steps
        is to rename `.js` and `.jsx` files to `.ts` and `.tsx`, respectively. This script aims to
        automate that process by identifying files that need to be renamed based on their content
        and then renaming them accordingly.
      </Typography>
      <Typography className="text-20 mt-20 mb-10 font-700" variant="h5">
      ...
      <Box mb={2}>
        <Paper>
          <Box p={2}>
            <code>
              npm install glob @babel/parser @babel/traverse @types/glob @types/node ts-node
            </code>
          </Box>
        </Paper>
      </Box>
      ...
      <Typography variant="body1" gutterBottom>
        Migrating from JavaScript to TypeScript can be a significant undertaking, especially in
        large projects. This script streamlines one of the initial, more tedious tasks, letting you
        focus on the nuances of adding types and enjoying the benefits of TypeScript sooner.
      </Typography>
    </>
  );
}
