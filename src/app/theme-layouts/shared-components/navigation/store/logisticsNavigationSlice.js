...
import logisticsNavigationConfig from 'app/configs/logisticsNavigationConfig';
...
const initialState = navigationAdapter.upsertMany(
	...
	FuseNavigationHelper.flattenNavigation(logisticsNavigationConfig)
...
} = navigationAdapter.getSelectors((state) => state.logisticsnavigation);
/**
 * The navigation slice
 */

export const logisticsNavigationSlice = createSlice({
	name: 'logisticsnavigation',
	initialState,
	reducers: {
...
rootReducer.inject(logisticsNavigationSlice);
logisticsNavigationSlice.injectInto(rootReducer);
export const { setNavigation, resetNavigation } = logisticsNavigationSlice.actions;
...
export default logisticsNavigationSlice.reducer;
