...
export function useCreateInspectionSchedule() {
	...
	return useMutation((formData) => createInspectionScheduleApi(formData), {
		onSuccess: (response) => {
			...
			if (response?.data?.success) {
				...
				toast.success(response?.data?.message || 'Inspection scheduled successfully!');
			}
		},
		onError: (error) => {
...
export function useGetMyInspectionSchedules() {
	return useQuery(['__myInspectionSchedules'], () => getMyInspectionSchedulesApi(), {
...
export function useCancelInspectionSchedule() {
	...
	return useMutation(({ scheduleId, reason }) => cancelInspectionScheduleApi(scheduleId, reason), {
		onSuccess: (response) => {
			// Invalidate and refetch inspection schedules
			queryClient.invalidateQueries('__myInspectionSchedules');
			queryClient.invalidateQueries('__myInspectionSchedulesCalendar');

			toast.success(response?.data?.message || 'Inspection cancelled successfully!');
		},
		onError: (error) => {
...
export function useRescheduleInspection() {
	...
	return useMutation(({ scheduleId, formData }) => rescheduleInspectionApi(scheduleId, formData), {
		onSuccess: (response) => {
			// Invalidate and refetch inspection schedules
			queryClient.invalidateQueries('__myInspectionSchedules');
			queryClient.invalidateQueries('__myInspectionSchedulesCalendar');

			toast.success(response?.data?.message || 'Inspection rescheduled successfully!');
		},
		onError: (error) => {
